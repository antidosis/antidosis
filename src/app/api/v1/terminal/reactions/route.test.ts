...
const mockTerminalMessageReactionFindUnique = vi.fn();
const mockTerminalMessageReactionDelete = vi.fn();
const mockTerminalMessageReactionCreate = vi.fn();
...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    terminalMessageReaction: {
      findUnique: (...args: unknown[]) => mockTerminalMessageReactionFindUnique(...args),
      delete: (...args: unknown[]) => mockTerminalMessageReactionDelete(...args),
      create: (...args: unknown[]) => mockTerminalMessageReactionCreate(...args),
    },
  },
}));
...
function makeRequest(body: unknown): NextRequest {
  return new Request("http://localhost/api/v1/terminal/reactions", {
    method: "POST",
...
describe("POST /api/v1/terminal/reactions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 403 for non-admin in staff channel", async () => {
    mockGetUser.mockResolvedValue({ data: { user: makeAuthUser() }, error: null });
    mockProfileFindUnique.mockResolvedValue({ id: "profile-1", email: "test@example.com" });
  ...
  it("returns 200 and removes existing reaction", async () => {
    ...
    mockTerminalMessageReactionFindUnique.mockResolvedValue({ id: "r1" });
    mockTerminalMessageReactionDelete.mockResolvedValue({});
  ...
  it("returns 201 and adds new reaction", async () => {
    ...
    mockTerminalMessageReactionFindUnique.mockResolvedValue(null);
    mockTerminalMessageReactionCreate.mockResolvedValue({ id: "r1" });
