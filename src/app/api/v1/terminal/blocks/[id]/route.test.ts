...
const mockBlockDeleteMany = vi.fn();
...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    block: {
      deleteMany: (...args: unknown[]) => mockBlockDeleteMany(...args),
    },
  },
}));
...
describe("DELETE /api/v1/terminal/blocks/[id]", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const res = await DELETE(makeRequest("http://localhost/api/v1/terminal/blocks/b1"), {
      params: { id: "b1" },
    });
  ...
  it("returns 200 on success", async () => {
    ...
    expect(mockBlockDeleteMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "b1", blockerId: "profile-1" },
      })
    );
  });
