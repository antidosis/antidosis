...
export async function generateContractPdf(data: ContractPdfData): Promise<Uint8Array> {
  ...
  y -= 14;
  drawText(`Generated: ${new Date(data.generatedAt).toLocaleString("en-AU")}`, {
    size: 9,
  ...
  if (data.needDeadline || data.needTimeRange || data.needLocation) {
    const metaItems = [];
    if (data.needDeadline)
      metaItems.push(`Deadline: ${new Date(data.needDeadline).toLocaleDateString("en-AU")}`);
  ...
  if (data.offerValue) {
    y -= 6;
    drawText(`Estimated value: $${data.offerValue.toLocaleString()}`, {
      size: 10,
      color: rgb(0.4, 0.4, 0.4),
    });
    y -= 14;
  ...
  // Unified statement
  drawText("Both parties have reviewed and agreed to the following terms:", {
    size: 10,
    color: rgb(0.4, 0.4, 0.4),
  });
  y -= 16;
  ...
  if (hasPartyATerms || hasPartyBTerms) {
    drawText("Terms proposed by Party A (Need Poster):", {
      size: 10,
      bold: true,
    ...
    if (data.partyAUseMessageTerms) {
      drawText("Terms derived from message thread (see Section 5)", {
        size: 10,
        color: rgb(0.4, 0.4, 0.4),
      });
      y -= 14;
    ...
    drawText("Terms proposed by Party B (Fulfiller):", {
      size: 10,
      bold: true,
      color: rgb(0.3, 0.3, 0.3),
    });
  ...
  for (const field of sharedTermFields) {
    if (field.value) {
      if (!hasSharedTerms) {
        drawText("Shared terms and conditions:", {
          size: 10,
          bold: true,
          color: rgb(0.3, 0.3, 0.3),
        });
  ...
  if (!hasPartyATerms && !hasPartyBTerms && !hasSharedTerms) {
    drawText(
      "No specific terms were recorded. Both parties agree to complete the exchange in good faith.",
  ...
  y -= 14;
  drawText(
    `Name: ${data.partyA.fullName || "N/A"}  |  Email: ${data.partyA.email}${data.partyA.isVerified ? "  |  ✓ Verified" : ""}`,
    { size: 9 }
  );
  ...
  y -= 14;
  drawText(
    `Name: ${data.partyB.fullName || "N/A"}  |  Email: ${data.partyB.email}${data.partyB.isVerified ? "  |  ✓ Verified" : ""}`,
    { size: 9 }
  );
  ...
  drawText(
    "By signing below, both parties acknowledge they have read, understood, and agree to be bound by the terms of this contract.",
  ...
  if (data.partyASignedAt) {
    ...
    drawText(`Signed: ${new Date(data.partyASignedAt).toLocaleString("en-AU")}`, {
      size: 9,
      color: rgb(0.2, 0.5, 0.2),
    });
  } else {
    drawText("Party A: ________________________________  Date: _______________", {
      size: 10,
      color: rgb(0.4, 0.4, 0.4),
    });
  }
  ...
  if (data.partyBSignedAt) {
    ...
    drawText(`Signed: ${new Date(data.partyBSignedAt).toLocaleString("en-AU")}`, {
      size: 9,
      color: rgb(0.2, 0.5, 0.2),
    });
  } else {
    drawText("Party B: ________________________________  Date: _______________", {
      size: 10,
      color: rgb(0.4, 0.4, 0.4),
    });
  }
  ...
  y -= 12;
  drawText(
    "Both parties agree to fulfil the terms outlined above. Disputes should be resolved through direct communication.",
